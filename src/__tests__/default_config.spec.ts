...
describe("should store cookies to cookiejar when default.jar was assigned",
  async (t) => {
...
test.serial(
  "should send cookies from cookiejar when default.jar was assigned",
  async (t) => {
    const { port } = await createTestServer([
      (req, res) => {
...
test.serial(
  "should use config.cookiejar in preference to default.jar",
  async (t) => {
    const { port } = await createTestServer([
      (req, res) => {
